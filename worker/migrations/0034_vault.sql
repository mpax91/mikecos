...
-- Defensive: if a prior attempt at this migration got partway through
-- before failing (as actually happened once in deploy — a leftover
-- entities_new blocked the retry with "table already exists"), this makes
-- a retry self-heal instead of getting permanently stuck. Safe regardless:
-- the real `entities` table is never touched until entities_new is fully
...
-- definition list.
CREATE TABLE IF NOT EXISTS vault_field_defs (
  id TEXT PRIMARY KEY,
  name TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS vault_field_groups (
  id TEXT PRIMARY KEY,
  name TEXT NOT NULL,
  created_at TEXT NOT NULL
);

CREATE TABLE IF NOT EXISTS vault_group_fields (
  id TEXT PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_vault_group_fields_group ON vault_group_fields(group_id);
...
CREATE TABLE IF NOT EXISTS vault_template_groups (
  id TEXT PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_vault_template_groups_template ON vault_template_groups(template_id);
...
CREATE INDEX IF NOT EXISTS idx_vault_entry_groups_entry ON vault_entry_groups(entry_id);
...
CREATE TABLE IF NOT EXISTS vault_field_values (
  id TEXT PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_vault_field_values_entry_group ON vault_field_values(entry_group_id);
...
CREATE INDEX IF NOT EXISTS idx_vault_categories_trigger ON vault_categories(trigger_field_def_id);
